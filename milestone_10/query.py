...
def placing_order(search_item, item_count_in_warehouse):
    ...
    if order_quantity <= total_item_count_in_Warehouses:
        ...
        for warehouse_count in item_count_in_warehouse:
            for warehouse in stock:
                if str(warehouse) == warehouse_count[0]:
                    print(
                        f"warehouse: {warehouse}, warehouse_count: {warehouse_count}")
        ...
        print("-"*100)
        ask_order_max = input(
            f"Do you want to order the {search_item} in maximum quantity of {total_item_count_in_Warehouses}? (y/n) -  {colors.ANSI_YELLOW} ")
                ...
                if str(warehouse) == warehouse_count[0]:
                    ...
                    for item in warehouse.stock:
                        if str(item).lower() == search_item.lower() and total_item_count_in_Warehouses > 0:
                            print(
                                f"order quantity: {total_item_count_in_Warehouses}")
                            warehouse.stock.remove(item)
...
def search_and_order_item(actions, authorized_employee):
    ...
    if len(location) > 0:
        ...
        if isinstance(authorized_employee, Employee) == True:
            place_order = input(
                f"Do you want to place an order for the item {search_item}? (y/n) - {colors.ANSI_YELLOW}")
            if place_order.lower() in ("y", "Y"):
                item_count_in_warehouse = sorted(item_count_in_warehouse_dict.items(
                ), key=lambda x: x[1], reverse=True)  # sort warehouse based on item counts
...
def category_selection(actions, authorized_employee):
            ...
            for warehouse in stock:
                for item in warehouse.stock:
                    if value_id == item.category:
                        ...
                        print(
                            f"{' ' * 25}{colors.ANSI_GREEN}{item.state} {item.category}, {warehouse}")
    ...
    if not category_name == None:
        ...
        print("."*120)
        actions.append(
            f"Browsed the category {category_name}.  {'-' * 10} {datetime.now()}")
    continue_session = input(
        f"\n{'*' * 20}  {colors.ANSI_PURPLE}Do you want to continue with another operation? (y/n){colors.ANSI_RESET}  {'*' * 20}   -   {colors.ANSI_YELLOW}")
    if continue_session in ("y", "Y"):
        run(actions, authorized_employee)
...
def run(actions, authorized_employee=None):
    ...
    if menu_selection == "1":
        ...
        actions.append(
            f"listed {total_items} items from {len(new_item_dict.keys())} Warehouses.  {'-' * 10} {datetime.now()}")
    # actions.append("Hi")
        continue_session = input(
            f"\n{'*' * 20}  {colors.ANSI_PURPLE}Do you want to continue with another operation? (y/n){colors.ANSI_RESET}  {'*' * 20}   -   {colors.ANSI_YELLOW}")
    ...
    # Else, if they pick 2
        ...
        search_and_order_item(actions, authorized_employee)
...
def start_shopping():
    ...
    # this can be either user object or employee object depending on the entry mode
