...
def placing_order(search_item, total_item_count_in_Warehouses):
    ...
    if order_quantity <= total_item_count_in_Warehouses:
        ...
        print('-' * 100)
        ask_order_max = input(
            f"Do you want to order the {search_item} in a maximum quantity of {total_item_count_in_Warehouses}? (y/n) - {colors.ANSI_YELLOW} ")
    ...
    if len(location) > 0:
        ...
        if isinstance(authorized_employee, Employee) == True:
            ...
            if place_order.lower() in ("y", "Y"):
                placing_order(search_item, sum(
                    item_count_in_warehouse_dict.values()))
    else:
...
def select_operation(actions, authorized_employee):
    ...
    if menu_selection == "1":
        ...
        actions.append(
            f"listed {total_items} items from {len(new_item_dict.keys())} Warehouses")
    # actions.append("Hi")
        continue_session = input(
            f"\n{'*' * 20}  {colors.ANSI_PURPLE}Do you want to continue with another operation? (y/n){colors.ANSI_RESET}  {'*' * 20}   -   {colors.ANSI_YELLOW}")
